...
import { RefreshCw, TrendingDown, Activity, BarChart2, ShieldAlert, Target, Zap, Waves, Info } from 'lucide-react';
...
export default function BearishScanner() {
    ...
    const getConfidenceColor = (conf) => {
        if (conf >= 85) return 'text-orange-600 bg-orange-50 border-orange-100';
        if (conf >= 60) return 'text-red-500 bg-red-50 border-red-100';
        return 'text-blue-600 bg-blue-50 border-blue-100';
    };

    const getConfidenceBarColor = (conf) => {
        if (conf >= 85) return 'bg-orange-500';
        if (conf >= 60) return 'bg-red-500';
        return 'bg-blue-500';
    };

    return (
        <div className="p-6 max-w-7xl mx-auto text-gray-900 font-sans">
            <header className="mb-12 flex flex-col md:flex-row justify-between md:items-end gap-8 bg-white/80 backdrop-blur-xl p-10 rounded-[3rem] shadow-2xl shadow-orange-100/50 border border-white">
                <div className="flex items-center gap-6">
                    <div className="bg-gradient-to-tr from-orange-600 via-red-500 to-orange-400 p-5 rounded-[2rem] shadow-2xl shadow-orange-200">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 text-gray-500">
                    <div className="relative mb-10">
                        <Waves className="w-24 h-24 text-orange-100 animate-[ping_2s_infinite]" />
                        <Activity className="w-12 h-12 text-orange-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse" />
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {setups.length > 0 ? (
                        setups.map((stock, idx) => (
                            <div key={idx} className="bg-white rounded-[3rem] border border-gray-100 p-8 hover:shadow-[0_30px_60px_-15px_rgba(249,115,22,0.15)] hover:border-orange-200 transition-all group relative overflow-hidden flex flex-col h-full">
                                {stock.score >= 8 && (
                                    <div className="absolute -right-12 -top-12 bg-orange-500 text-white w-28 h-28 flex items-end justify-center pb-4 rotate-45 font-black text-[10px] uppercase tracking-tighter shadow-xl">
                                        High Accuracy
                                    </div>
                                )}
                                ...
                                <div className="flex justify-between items-start mb-8">
                                    <div>
                                        <h2 className="text-4xl font-black text-gray-900 tracking-tighter group-hover:text-orange-600 transition-colors uppercase">{stock.symbol}</h2>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="text-gray-400 font-bold text-sm tracking-tight text-lg">₹{stock.price}</span>
                                            {stock.htf_alignment && (
                                                <span className="bg-orange-50 text-orange-600 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter border border-orange-100">
                                                    Bear Trend OK
                                                </span>
                                            )}
                                        </div>
                                ...
                                <div className="space-y-6 flex-grow">
                                    <div className="space-y-2">
                                        ...
                                        <div className="h-3 w-full bg-gray-50 rounded-full overflow-hidden border border-gray-100">
                                            <div
                                                className={`h-full transition-all duration-1000 ${getConfidenceBarColor(stock.confidence)} shadow-[0_0_10px_rgba(249,115,22,0.3)]`}
                                        ...
                                        <div className="space-y-3">
                                            {stock.checklist?.map((item) => (
                                                <div key={item.id} className="flex items-center gap-4">
                                                    <div className={`w-5 h-5 rounded-lg flex items-center justify-center text-[10px] shadow-sm ${item.status ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-400'}`}>
                                                        {item.status ? '✓' : '•'}
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className={`text-xs font-bold leading-tight ${item.status ? 'text-gray-900' : 'text-gray-300'}`}>{item.label}</span>
                                                        {item.status && <span className="text-[10px] text-orange-600 font-bold opacity-80">{item.detail}</span>}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-dashed border-gray-100">
                                    <div className="bg-orange-50/30 p-4 rounded-2xl border border-orange-50 text-center">
                                        <p className="text-[10px] font-extrabold text-orange-400 uppercase tracking-tighter">Exit SL</p>
                                        <p className="text-xl font-black text-orange-600 tracking-tighter">₹{stock.stop_loss.toFixed(1)}</p>
                                    </div>
                                    <div className="bg-red-50/30 p-4 rounded-2xl border border-red-50 text-center">
                                        <p className="text-[10px] font-extrabold text-red-400 uppercase tracking-tighter">Target (AI)</p>
                                        <p className="text-xl font-black text-red-600 tracking-tighter">₹{stock.target.toFixed(1)}</p>
                                    </div>
                                </div>
