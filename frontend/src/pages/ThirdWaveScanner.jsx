...
export default function ThirdWaveScanner() {
    ...
    const fetchSetups = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/third-wave?timeframe=${timeframe}`);
            if (response.data.status === 'success') {
                setSetups(response.data.data);
            }
        } catch (err) {
            console.error(err);
            setError('Failed to fetch 3rd Wave setups. Ensure backend is running.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto text-gray-900 font-sans min-h-screen">
            <header className="mb-6 md:mb-10 flex flex-col xl:flex-row justify-between xl:items-end gap-6 bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-gray-100 relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 md:gap-6 relative z-10">
                    <div className="bg-gradient-to-br from-indigo-700 to-blue-600 p-3 md:p-5 rounded-2xl md:rounded-3xl shadow-xl shadow-indigo-100/50">
                    ...
                    <div>
                        <h1 className="text-2xl md:text-4xl font-black text-gray-900 tracking-tight flex flex-wrap items-center gap-2 md:gap-4">
                            3rd Wave Strategy
                            <span className="bg-blue-100 text-blue-700 text-[9px] md:text-[10px] font-black px-2 md:px-3 py-0.5 md:py-1 rounded-full uppercase tracking-widest border border-blue-200">Impulse</span>
                        </h1>
                        <p className="text-gray-500 text-xs md:text-sm font-medium mt-1">High-Probability Trend Following Model</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4 relative z-10">
                    ...
                    >
                        ...
                        {loading ? 'Calculating' : 'Scan 3rd Wave'}
                    </button>
                </div>
            </header>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-32 md:py-48 text-gray-500 text-center">
                    <Activity className="w-16 h-16 md:w-24 md:h-24 mb-6 md:mb-10 animate-pulse text-indigo-600" />
                    <p className="text-2xl md:text-4xl font-black text-indigo-900 tracking-tight">Calculating Velocity...</p>
                    <p className="text-gray-400 font-bold mt-2 uppercase tracking-widest text-[10px] md:text-xs">Identifying Impulse & Correction Phases</p>
                </div>
            ) : (
                    ...
                    {setups.length > 0 ? (
                            ...
                            <div key={idx} className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden flex flex-col h-full">
                                <div className="flex justify-between items-start mb-6 md:mb-8 relative z-10">
                                    <div className="max-w-[65%]">
                                        <div className="flex items-center gap-2 md:gap-3">
                                            ...
                                            <span className={`text-[8px] md:text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter border shrink-0 ${stock.type.includes('BULL') ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                {stock.type.includes('BULL') ? 'Bull' : 'Bear'}
                                            </span>
                                        </div>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="flex items-center justify-end gap-1 mt-1">
                                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${stock.score >= 4 ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`}></div>
                                            <span className="text-[9px] md:text-[11px] font-black text-gray-500 uppercase tracking-widest italic">Win: {Math.round((stock.score / 5) * 100)}%</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 mb-6 md:mb-8 flex-grow relative z-10">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                                        <div className="bg-rose-50/40 p-3 md:p-4 rounded-2xl md:rounded-3xl border border-rose-100/50 flex flex-col items-center text-center">
                                            <p className="text-[8px] md:text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1 flex items-center gap-1 shrink-0">
                                                <Target className="w-2.5 h-2.5 md:w-3 md:h-3" /> Stop
                                            </p>
                                        ...
                                        <div className="bg-emerald-50/40 p-3 md:p-4 rounded-2xl md:rounded-3xl border border-emerald-100/50 flex flex-col items-center text-center">
                                            <p className="text-[8px] md:text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1 flex items-center gap-1 shrink-0">
                                                <TrendingUp className="w-2.5 h-2.5 md:w-3 md:h-3" /> Target
                                            </p>
                                ...
                                <div className="flex justify-between items-center text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest pt-4 md:pt-6 border-t border-gray-50 relative z-10">
                                    <div className="flex items-center gap-2">
                                        <BarChart2 className="w-4 h-4 text-indigo-500 opacity-50" />
                                        <span>Vol: {(stock.volume / stock.avg_volume).toFixed(1)}x</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 bg-indigo-50/50 text-indigo-600 px-3 py-1 rounded-full border border-indigo-100/30 font-black">
                                        <Zap className="w-3 md:w-4 h-3 md:h-4 text-amber-500" />
                                        <span>Wave 3 Launch</span>
                                    </div>
                                </div>
                            </div>
                        ))
                        ...
                        <div className="col-span-full bg-white p-12 md:p-24 rounded-[2rem] md:rounded-[4rem] border border-gray-100 text-center shadow-sm flex flex-col items-center">
                            <div className="bg-gray-50 p-6 md:p-10 rounded-full mb-6 md:mb-10 group-hover:scale-110 transition-transform">
                                <Zap className="w-12 h-12 md:w-20 md:h-20 text-gray-200" />
                            </div>
                            <h3 className="text-2xl md:text-4xl font-black text-gray-900 mb-4 tracking-tighter uppercase">No 3rd Waves Found</h3>
                            <p className="text-gray-500 font-bold max-w-sm mx-auto leading-relaxed text-sm md:text-lg">The AI is currently monitoring Wave 2 corrections for 80+ stocks. No breakouts into Wave 3 detected on {timeframe}.</p>
                        </div>
                    )}
                </div>
            )}
