        ...
        <div className="p-6 max-w-7xl mx-auto text-gray-900 font-sans min-h-screen bg-gray-50/50">
            <header className="mb-8 flex flex-col md:flex-row justify-between md:items-end gap-6 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                <div className="flex items-center gap-5">
                    <div className="bg-gradient-to-br from-indigo-700 to-blue-600 p-4 rounded-2xl shadow-lg shadow-indigo-100">
                        <Zap className="text-white w-9 h-9" />
                    </div>
                    <div>
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                            3rd Wave Strategy
                            <span className="bg-blue-100 text-blue-700 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest border border-blue-200">Elliott Wave</span>
                        </h1>
                        <p className="text-gray-500 font-medium mt-1">High-Probability Trend Following Model</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    >
                        ...
                        {loading ? 'Analyzing Waves...' : 'Scan 3rd Wave'}
                    </button>
                </div>
            </header>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 text-gray-500">
                    ...
                    <p className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-violet-600">Calculating Wave Velocity...</p>
                    <p className="text-sm mt-3 text-gray-400 font-bold uppercase tracking-widest">Identifying Impulse and Corrective Phases</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {setups.length > 0 ? (
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    <div>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter border ${stock.type.includes('BULL') ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                                {stock.type.includes('BULL') ? 'Bullish Impulse' : 'Bearish Impulse'}
                                            </span>
                                        </div>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="flex items-center justify-end gap-1 mt-1">
                                            <div className={`w-2 h-2 rounded-full ${stock.score >= 4 ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`}></div>
                                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Prob: {Math.round((stock.score / 5) * 100)}%</span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="bg-emerald-50/40 p-3 rounded-xl border border-emerald-100/50">
                                            <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1 flex items-center gap-1">
                                                <TrendingUp className="w-3 h-3" /> Target (1.62)
                                            </p>
                        ...
                        <div className="col-span-full bg-white p-24 rounded-[3rem] border border-gray-100 text-center shadow-xl shadow-gray-100/50 flex flex-col items-center">
                            ...
                            <h3 className="text-3xl font-black text-gray-900 mb-3 tracking-tighter uppercase">No 3rd Waves Found</h3>
                            <p className="text-gray-500 font-bold max-w-sm mx-auto leading-relaxed">The AI is currently monitoring the corrective phase (Wave 2) for 80+ stocks. No breakouts into Wave 3 detected on {timeframe}.</p>
                        </div>
                    )}
                </div>
            )}
